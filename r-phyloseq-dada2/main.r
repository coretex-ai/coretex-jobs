...
conda_environment_path <- args[1]

library(reticulate)
use_condaenv(conda_environment_path)
...
main <- function(experiment) {
    ...
    for (sample in experiment$parameters[["dataset"]]$samples) {
        ...
        if (startsWith(sample$name, "_metadata")) {
            next
        }
    ...
    # Assume filenames have format: SAMPLENAME_XXX.fastq

    sample_names <- sapply(
        strsplit(basename(forward_read_paths), "_"),
        function(x) x[1]
    )
    ...
    max_ee_forward <- experiment$parameters[["trimRightReverse"]]
    max_ee_reverse <- experiment$parameters[["trimRightReverse"]]
    ...
    track_matrix_path <- file.path(
        output_path,
        "track_trimLeft_trimRight_10_90_maxEE_2_4.csv"
    )
    ...
    metadata_path <- builtins$str(
        experiment$dataset$getSample("_metadata")$joinPath("metadata.csv")
    )
    map <- read.table(
        metadata_path,
        sep = ",",
