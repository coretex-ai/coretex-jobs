...
def print_args(args: Optional[dict] = None, show_file=True, show_fcn=False):
    ...
    logging.info(colorstr(s) + ', '.join(f'{k}={v}' for k, v in args.items()))
...
def check_git_status():
    ...
    logging.info(emojis(s))  # emoji-safe
...
def check_requirements(requirements=ROOT / 'requirements.txt', exclude=(), install=True):
    ...
    for r in requirements:
        try:
            ...
            if install:
                logging.info(f"{s}, attempting auto-update...")
                try:
                    assert check_online(), f"'pip install {r}' skipped (offline)"
                    logging.info(check_output(f"pip install '{r}'", shell=True).decode())
                    n += 1
                except Exception as e:
                    logging.info(f'{prefix} {e}')
            else:
                logging.info(f'{s}. Please install and rerun your command.')

    if n:  # if packages updated
        source = file.resolve() if 'file' in locals() else requirements
        s = f"{prefix} {n} package{'s' * (n > 1)} updated per {source}\n" \
            f"{prefix} ⚠️ {colorstr('bold', 'Restart runtime or rerun command for updates to take effect')}\n"
        logging.info(emojis(s))
...
def check_img_size(imgsz, s=32, floor=0):
    ...
    if new_size != imgsz:
        logging.info(f'WARNING: --img-size {imgsz} must be multiple of max stride {s}, updating to {new_size}')
    return new_size
...
def check_imshow():
    ...
    try:
        ...
        logging.info(f'WARNING: Environment does not support cv2.imshow() or PIL Image.show() image displays\n{e}')
        return False
...
def check_font(font=FONT):
    ...
    if not font.exists() and not (CONFIG_DIR / font.name).exists():
        ...
        logging.info(f'Downloading {url} to {CONFIG_DIR / font.name}...')
        ...
        if not all(x.exists() for x in val):
            ...
            if s and autodownload:  # download script
                ...
                if s.startswith('http') and s.endswith('.zip'):  # URL
                    ...
                    logging.info(f'Downloading {s} to {f}...')
                ...
                logging.info(emojis(f"Dataset download {s}"))
            else:
        ...
        if Path(url).is_file():  # exists in current path
            ...
            for i in range(retry + 1):
                ...
                if success:
                    ...
                    logging.info(f'Download failure, retrying {i + 1}/{retry} {url}...')
                else:
...
def strip_optimizer(f='best.pt', s=''):  # from object_detection.utils.general import *; strip_optimizer()
...
def print_mutation(results, hyp, save_dir, prefix=colorstr('evolve: ')):
    ...
    # Print to screen
    logging.info(prefix + f'{generations} generations finished, current result:\n' + prefix +
