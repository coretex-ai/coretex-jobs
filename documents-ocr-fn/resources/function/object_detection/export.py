...
from object_detection.custom.custom_datasets import LoadImages
from object_detection.utils.general import (LOGGER, check_dataset, check_img_size, check_requirements, check_version, colorstr,
...
def export_engine(model, im, file, train, half, simplify, workspace=4, verbose=False, prefix=colorstr('TensorRT:')):
    ...
    try:
        ...
        logging.info(f'{prefix} building FP{16 if builder.platform_has_fast_fp16 else 32} engine in {f}')
...
def export_saved_model(model,
    ...
    try:
        ...
        # fix for tf SavedModel export failure due to cublas failing to initialize math mode
        tf.config.experimental.enable_tensor_float_32_execution(False)

        from tensorflow.python.framework.convert_to_constants import convert_variables_to_constants_v2

        from object_detection.models.tf import TFDetect, TFModel
...
def export_tflite(keras_model, im, file, int8, data, nms, agnostic_nms, prefix=colorstr('TensorFlow Lite:')):
    ...
    try:
        ...
        if int8:
            from object_detection.models.tf import representative_dataset_gen
