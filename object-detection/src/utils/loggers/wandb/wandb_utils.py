...
from custom.custom_datasets import LoadDataset  #, img2label_paths
...
class WandbLogger():
    ...
    def log_model(self, path, opt, epoch, fitness_score, best_model=False):
        ...
        logging.info(f"Saving model artifact on epoch {epoch + 1}")
    ...
    def log_dataset_artifact(self, data_file, single_cls, project, overwrite_config=False):
        ...
        # log train set
        if not log_val_only:
            self.train_artifact = self.create_dataset_table(LoadDataset(data['train'], rect=True, batch_size=1),
                                                            names,
        ...
        self.val_artifact = self.create_dataset_table(
            LoadDataset(data['val'], rect=True, batch_size=1), names, name='val') if data.get('val') else None
        if data.get('val'):
        ...
        if not log_val_only:
            ...
            with open(path, 'w') as f:
                ...
                logging.info(f"Created dataset config file {path}")
    ...
    def create_dataset_table(self, dataset: LoadDataset, class_to_id: Dict[int, str], name: str = 'dataset'):
        """
